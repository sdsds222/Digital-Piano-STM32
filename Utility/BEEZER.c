...
#define BEEZER_PSC 7199
int music[15] = {75, 67, 60, 56, 50, 44, 40, 38, 34, 30, 28, 25, 22, 20, 19};
void BEEZER_Init(uint16_t arr, uint16_t psc)
{
    ...
    // 设置ARR（自动重载寄存器）的值，该值决定了计数器的最大值
    //ARR
    TIM_TimeBaseInitStructure.TIM_Period = arr;

    // 设置PSC（预分频寄存器）的值，该值决定了时钟分频的因子
    //PSC
    ...
    //初始化OC3。TIM3的CH1通道复用了PB0的引脚,输出PWM
...
void BEEZER_Play(uint16_t t, uint8_t mode)
{
    if(mode > 0) {
        t += 7;
    }

    uint16_t t1 = music[t]/2  + 1;
    BEEZER_Init(music[t], BEEZER_PSC);

    //设置CCR捕获比较寄存器Capture/Compare Register的值
    TIM_SetCompare3(TIM3, t1);
}
